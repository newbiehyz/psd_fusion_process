...
// 时间同步检查
inline bool CheckTimeSync(uint64_t current1970_ms, uint64_t rd_timestamp, uint64_t dr_timestamp) {
...
// RD帧时间戳检查
inline bool CheckRDFrameTimestamp(uint64_t currentTimestamp) {
...
// 清除RD
inline void ClearRD(rd::QuadParkingSlots& rd_info) {
    rd_info = rd::QuadParkingSlots{};
    LOGD("CLEAR rd_info");
}

// 清除DR
inline void ClearDR(Loc::App2emap_DR& dr_pose, padVehiclePose& pose_globaldata) {
...
// 清除障碍物
inline void ClearOBS(Fus::PkEmapObs& obs_info_get) {
    obs_info_get = Fus::PkEmapObs{};
    LOGD("CLEAR obs_info_get");
}

// 清除USS
inline void ClearUSS(UssIf_stPLVOutputInfo_t& uss_info, UssIf_stPLVOutputInfo_t& uss_restruct) {
...
// 清除所有车位
inline void ClearParkingSlots(std::vector<padVisionSlotCoord>& singleframeslots, 
...
// 记录停车位信息
inline void LogSlotInfo(const apaSlotListInfo& slots, const std::string& slotType) {
...
// 记录世界坐标停车位信息
inline void LogWorldSlotInfo(const apaSlotListInfo& slots, const std::string& slotType) {
...
// 清除选择和推荐停车位
inline void ClearSelectRecommendSlot(int& HMI_temp_ID, int& HMI_select_ID, int& VCU_select_ID_ON, int& final_select_ID, int& RECOMMEND_ID, int& final_ID, int apa_status) {
    HMI_temp_ID = 0;
    HMI_select_ID = 0;
    VCU_select_ID_ON = 0;
    final_select_ID = 0;
