...
void filterSlotPosition(Sfus::FusionSlotInfo& slot, const Sfus::FusionSlotInfo& prevSlot) {
    for (int i = 0; i < 4; i++) {
        if (std::abs(slot.pt[i].x - prevSlot.pt[i].x) < POSITION_THRESHOLD) {
            slot.pt[i].x = (slot.pt[i].x + prevSlot.pt[i].x) / 2.0f; // 取均值
        }
        if (std::abs(slot.pt[i].y - prevSlot.pt[i].y) < POSITION_THRESHOLD) {
