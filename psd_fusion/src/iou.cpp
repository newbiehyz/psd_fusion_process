...
Point Line::intersection(const Line &line, bool *bOnEdge) const
{
    ...
    if (p1 == p2 && line.p1 == line.p2){
        ...
        if (ang < EPS || fabs(3.141592653 - ang) < EPS)
            bOn = false; // Collinear!!
        else {
...
void changePoint(apaSlotInfo rect, Vertexes &vert)
{
    Vertexes _vert;
    _vert.reserve(4);
    _vert.push_back(IOU::Point(rect.rectInfo.pt[0].x, rect.rectInfo.pt[0].y));
    _vert.push_back(IOU::Point(rect.rectInfo.pt[1].x, rect.rectInfo.pt[1].y));
    _vert.push_back(IOU::Point(rect.rectInfo.pt[2].x, rect.rectInfo.pt[2].y));
    _vert.push_back(IOU::Point(rect.rectInfo.pt[3].x, rect.rectInfo.pt[3].y));
...
WiseType whichWiseEx(const Vertexes &C)
{
    ...
    if (N > 2) {
        ...
        Point p01 = p1 - p0;
        Point p12 = p2 - p1;
        if ((fabs(p01^p12) <= EPS) && p01*p12 < 0.0)
            return NoneWise;
        else
