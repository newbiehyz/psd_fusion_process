...
void transformPLANNINGTARGETSLOT(Sfus::SlotCorners& slotCorners) {

    float oldA_x = slotCorners.cornerA.x;
    float oldA_y = slotCorners.cornerA.y;
    float oldB_x = slotCorners.cornerB.x;
    float oldB_y = slotCorners.cornerB.y;
    float oldC_x = slotCorners.cornerC.x;
    float oldC_y = slotCorners.cornerC.y;
    float oldD_x = slotCorners.cornerD.x;
    float oldD_y = slotCorners.cornerD.y;

    slotCorners.cornerA.x = oldA_y;
    slotCorners.cornerA.y = -oldA_x;
    
    slotCorners.cornerB.x = oldB_y;
    slotCorners.cornerB.y = -oldB_x;
    
    slotCorners.cornerC.x = oldC_y;
    slotCorners.cornerC.y = -oldC_x;
    
    slotCorners.cornerD.x = oldD_y;
    slotCorners.cornerD.y = -oldD_x;
}
...
tResult cpsd_fusion_process::TimeTrigger_thread_100ms_1()
{
    ...
    LOGD("PSD Version: 01191343 emos10.0.1 FOR 0122 RELEASE. update stopdis while GUIDANCE, relax stopdis threshold to 8000");
    // GET方式获取
    rd::QuadParkingSlots rd_info;
    unsigned long long singleframeslotsID;
    std::vector<padVisionSlotCoord> singleframeslots;
    ...
    if (psd2location.slotNum > 0){
        ...
        for (auto& psd_m_output : outputSlot_FUSED.slots_in_cur_frame){
            ...
            if (j == 0){
                ...
                if (final_ID != 0){
                    psd2location.fusionSlotInfo[j].slotInnerObType = 1;
                }else{
                    psd2location.fusionSlotInfo[j].slotInnerObType = 0;
                }
            }
            else{
    ...
    // 保存目标车位数据
    static Sfus::SfusionSlots saved_target_slot;
    static int saved_target_slot_fusionSlotType = 0;
    static bool has_saved_target_slot = false;
        ...
        if (apa_status != 5){ 
            for (int i = 0; i < outputSlot_FUSED.slots_in_cur_frame.size();++i){
                if (final_ID == outputSlot_FUSED.slots_in_cur_frame[i].rectInfo.label){ // 找到目标车位
                    ...
                    // 保存当前的目标车位
                    saved_target_slot = psd2planning.targetSlot;
                    saved_target_slot_fusionSlotType = target_slot_fusionSlotType;
                    has_saved_target_slot = true;
                }
            }
        }
    ...
    if (parkout_flag != 1){
        ...
        if (psd2planning.targetSlot.slotCorners.cornerA.x != 0){
            ...
            psd2perception.slotID = psd2planning.targetSlot.slotID;
